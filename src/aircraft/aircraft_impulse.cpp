                                              ...
                                              Eigen::Vector3d r) 
{
    ...
    Vector<double, 6> momentum = massMatrix * state.getX();
    Vector3d obj_vel_world = state.getX().head<3>() + state.getX().tail<3>().cross(r) + speed;
    Vector3d obj_linear_momentum = m * speed;
    Vector<double, 6> obj_momentum;
    reduceMass(m,r);
    obj_momentum << obj_linear_momentum, r.cross(obj_linear_momentum);
    Vector<double, 6> newX = invMassMatrix * (momentum - obj_momentum);
    state.setX(newX);
    return R_bn * obj_vel_world;
}